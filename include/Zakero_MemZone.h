...
#define ZAKERO_MEMZONE__ERROR_DATA \
	...
	X(Error_Resize_Too_Small           , 19 , "The resize request was too small to succeed"          ) \
...
[[]]          void        Zakero_MemZone_DefragEnable(Zakero_MemZone&, uint64_t) noexcept;
...
{
	bool is_last = false
		| block_is_last_(block_dst)
		| block_is_last_(block_src)
		;
	memcpy(zakero_memzone_block_data_(block_dst)
		, zakero_memzone_block_data_(block_src)
		, std::min(block_dst->size, block_src->size)
		);
	...
	{
		block_last_set_(block_dst, is_last);
	}
	...
	) noexcept
{
	...
	else // block_is_last_(block) == true
	{
...
// TODO: Rename to block_merge_with_prev_()
	...
	) noexcept
{
	...
	block_free = block_move_(block_to_move, block_free); // TODO: Rename block_free_swap_()
...
static void defrag_multi_pass_(Zakero_MemZone_Block* block
	) noexcept
{
	while(block != nullptr)
	{
...
TEST_CASE("/c/init/") // {{{
{
	...
	SUBCASE("Initialized") // {{{
	{
		...
		CHECK(block                         != nullptr);
		CHECK(block_is_acquired_(block)  == false);
		CHECK(block_is_allocated_(block) == false);
		CHECK(block_is_free_(block)      == true);
		CHECK(block_is_last_(block)      == true);
...
TEST_CASE("/c/defragnow/") // {{{
{
	...
	CHECK(ptr_2 == Zakero_MemZone_Acquire(memzone, id_2));
...
// }}}
// {{{ Zakero_MemZone_DefragEnable() -------- Check Block Content -
 ...
 * }}
 */
void Zakero_MemZone_DefragEnable(Zakero_MemZone& memzone
	, uint64_t defrag
	) noexcept
{
#if ZAKERO_MEMZONE_VALIDATE_IS_ENABLED // {{{
...
TEST_CASE("/c/defragenable/") // {{{
{
	...
	error = Zakero_MemZone_Allocate(memzone, mem_size, id_1);
	CHECK(error == Zakero_MemZone_Error_None);
	CHECK(ptr_2 == Zakero_MemZone_Acquire(memzone, id_1));
	Zakero_MemZone_Release(memzone, id_1);
...
#		endif // }}}
	...
	if(size < block->size) // Shrink
	{
		...
		memzone_block_shrink_(memzone, block, size);
	}
	else
	{
...
TEST_CASE("/c/resize/") // {{{
{
	...
	SUBCASE("Resize: Error") // {{{
	{
		...
		CHECK(error == Zakero_MemZone_Error_Resize_Too_Small);
...
TEST_CASE("/c/resize/defrag/") // {{{
{
	...
	SUBCASE("Resize: Smaller") // {{{
	{
		...
		error = Zakero_MemZone_Resize(memzone, id_x, mem_size / 2);
		CHECK(ptr_x == Zakero_MemZone_Acquire(memzone, id_x));
		CHECK(ptr_2 != Zakero_MemZone_Acquire(memzone, id_2));
	...
	SUBCASE("Resize: Larger") // {{{
	{
		...
		CHECK(ptr_x != Zakero_MemZone_Acquire(memzone, id_x));
		CHECK(ptr_x == Zakero_MemZone_Acquire(memzone, id_2));
