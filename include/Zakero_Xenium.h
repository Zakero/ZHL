...
#undef X
	...
	std::string to_string(const Xenium::PointerAxis&) noexcept;
	...
	) noexcept
{
	...
	return output_data.map[output_id];
}
...
{
	Xenium::VectorOutputId vector(output_data.map.size());
	vector.clear();
	...
	) noexcept
{
	...
	xenium->windowModeSet(window_id
